const getOtherDetails = async (userName) => {
    try {
        ...
        if (!res.ok) {
            // Handle other types of errors like rate limiting (e.g., 429)
        ...
        console.log(`Failed to fetch other details for ${userName}:`, error);
...
const getDetails = async (userName) => {
    try {
        ...
        console.log(`Failed to fetch details for ${userName}:`, error);
        const errorElement = document.querySelector('#error_message');
        errorElement.innerText = `Error: ${error.message}`;
        return null;
    }
...
const displayDetails = (details) => {
    ...
    details.forEach(detail => {
        if (detail.error) {
            ...
            detailsHTML += `<div> <h5>${detail.userName}</h5><p>Problems solved: ${detail.solvedProblem}<br> Easy: ${detail.easySolved}, Medium: ${detail.mediumSolved}, Hard: ${detail.hardSolved}<br> Today's submissions: ${detail.questionsSubmitted}</p></div>`;
...
const addLeetCodeID = async (userName) => {
    ...
    if (userNames.length < 7) {
        ...
        alert('You have reached the maximum limit of 7 LeetCode IDs.');
    }
};
...
const handleFormSubmit = async (event) => {
    ...
    if (userName) { // Only proceed if username is not empty
        ...
        const userNames = JSON.parse(localStorage.getItem('userNames')) || [];
        if (userNames.length === 7) {
            document.querySelector('#nameForm').style.display = 'none'; // Hide form if 7 IDs are stored
        }
